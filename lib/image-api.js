const { buildContentRequest, normalizeContentResponse, mergeImagePayload, imageDataUrl } = require("./provider-protocol");
...
function requestPhase(pathname, method = "GET") {
  ...
  if (pathname.startsWith("/images/tasks/")) return "generation_poll";
...
function networkNextActions(category) {
  ...
  const actions = {
    dns_resolution: ["检查DNS解析、代理或企业网络策略是否允许访问配置的API域名"],
    connection_refused: ["检查代理、防火墙或安全软件是否拒绝到配置的API地址的连接"],
...
async function readResponseBody(response, maxBytes = 100 * 1024 * 1024) {
  if (Number(response.headers.get("content-length")) > maxBytes) {
    await response.body?.cancel();
    throw new ApiError("上游响应超过大小限制", { code: "response_too_large" });
  }
  const chunks = [];
  let total = 0;
  if (response.body) {
    for await (const chunk of response.body) {
      total += chunk.length;
      if (total > maxBytes) throw new ApiError("上游响应超过大小限制", { code: "response_too_large" });
...
function buildGenerationPayload(options) {
  ...
  if (!Number.isInteger(n) || n < 1 || n > 4) {
    throw new InputError("图片数量必须在1到4之间", "invalid_count");
  }

  const model = String(options.model || "").trim();
  if (!model) throw new InputError("必须配置生图模型model", "model_required");
  ...
  if (
    ...
    const compression = Number(options.outputCompression);
    if (!Number.isInteger(compression) || compression < 0 || compression > 100) {
      throw new InputError("输出压缩率必须在0到100之间", "invalid_compression");
    }
...
function parseImageResult(body, headers) {
  ...
  const images = body.data.map((item, index) => {
    ...
    if (item?.url) {
      const parsed = imageDataUrl(item.url);
      if (parsed?.b64_json) return { type: "base64", value: parsed.b64_json, index };
      if (parsed?.url) return { type: "url", value: parsed.url, index };
    }
...
function createImageApi(config, fetchImpl = globalThis.fetch) {
  ...
  const format = config.apiFormat || "openai-images";
  const endpoints = { models: "/models", generations: "/images/generations", edits: "/images/edits", chat: "/chat/completions", generate_content: "/models/{model}:generateContent", ...config.endpoints };
  ...
  async function request(pathname, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt += 1) {
      ...
      try {
        ...
        const authHeaders = authType === "none" ? {} : authType === "header"
          ? { [config.authHeader || "api-key"]: config.apiKey }
        ...
        if (response.status >= 300 && response.status < 400) {
          await response.body?.cancel();
          throw new ApiError("API返回重定向；请直接配置最终Base URL，避免向其他地址转发凭据", { status: response.status, code: "api_redirect_blocked" });
        }
        const body = await readResponseBody(response, phase === "model_catalog" ? 5 * 1024 * 1024 : 100 * 1024 * 1024);
  ...
  async function contentRequest(options) {
    if (options.referenceImage) decodeReferenceImage(options.referenceImage);
    const merged = { model: config.model, ...options };
    // Reuse strict prompt/count validation without imposing Images API fields.
    buildGenerationPayload(merged);
    const payload = buildContentRequest(format, merged, config.extraBody);
    const pathname = format === "gemini"
      ? endpoints.generate_content.replace("{model}", encodeURIComponent(String(merged.model).replace(/^models\//, "")))
      : endpoints.chat;
    const response = await request(pathname, {
      method: "POST", body: payload, timeoutMs: options.timeoutMs || config.timeoutMs || 300000,
      maxRetries: 0, phase: options.referenceImage ? "edit_submit" : "generation_submit",
    });
    if (response.body?.error) throw createHttpError(response.body, { status: 400, headers: response.headers }, 1);
    const normalized = normalizeContentResponse(format, response.body);
    if (!normalized.data.length) {
      const blocked = response.body?.promptFeedback?.blockReason || response.body?.candidates?.some((c) => ["SAFETY", "IMAGE_SAFETY", "PROHIBITED_CONTENT"].includes(c.finishReason));
      throw new ApiError(blocked ? "供应商拒绝了本次内容请求" : "供应商未返回可保存的图片；请确认模型支持图片输出和响应参数", { code: blocked ? "content_blocked" : "empty_image_response", phase: options.referenceImage ? "edit_submit" : "generation_submit" });
    }
    return { ...parseImageResult(normalized, response.headers), attempts: response.attempts, transport: format };
  }

  return {
    async listModels(options = {}) {
      if (config.probeModels === false) {
        const result = { models: config.model ? [config.model] : [], probe: { status: 0, attempts: 0, skipped: true } };
        return options.includeMetadata ? result : result.models;
      }
      const response = await request(endpoints.models, {
        phase: "model_catalog",
      ...
      const models = Array.isArray(body?.data)
        ...
        : Array.isArray(body?.models) ? body.models.map((item) => item?.name?.replace(/^models\//, "")).filter(Boolean) : null;
      if (!models) throw new ApiError("模型目录返回无效结构", { code: "invalid_model_catalog", phase: "model_catalog" });
    ...
    async generate(options) {
      if (format !== "openai-images") return contentRequest(options);
      const payload = mergeImagePayload(buildGenerationPayload({ model: config.model, ...options }), config);
      const { body, headers, attempts } = await request(endpoints.generations, {
        ...
        shouldRetry: (error) => ["upstream_channel_unavailable", "rate_limit"].includes(error.code),
      });
    ...
    async edit(options) {
      if (format !== "openai-images") return contentRequest(options);
      const payload = mergeImagePayload(buildGenerationPayload({ model: config.model, ...options }), config, true);
      const reference = decodeReferenceImage(options.referenceImage);
      const formData = new FormData();
      const orderedFields = ["prompt", ...Object.keys(payload).filter((key) => key !== "prompt")];
      ...
      const { body, headers, attempts } = await request(endpoints.edits, {
        phase: "edit_submit",
        method: "POST",
        formData,
        timeoutMs: options.timeoutMs || config.timeoutMs || 300000,
