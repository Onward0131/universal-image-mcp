...
很多第三方API平台将聊天、代码和生图划分到不同分组，生图往往需要在专用分组下额外创建Key。分组常见名称为`image`、“生图”或“额外生图”；普通模型分组的Key不一定具备图片生成权限。

配置前，在供应商控制台确认生图分组允许调用你要使用的图片模型，再创建或选用该分组的Key，并填写与该Key对应的Base URL。若已误填普通Key，可用安装器的`-ResetApiKey`更新。仅修改MCP中的模型名称不会改变平台为Key授予的权限。

分组机制和名称由供应商决定，不是本MCP的统一要求。官方API或不采用分组的平台，按其账号、项目和Key的图片模型权限设置操作。鉴权成功或能够读取模型目录，也不保证该Key可以调用生图接口。
